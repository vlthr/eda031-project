...
std::vector<std::pair<int, std::string>> Inmemdb::list_newsgroups() {
...
bool Inmemdb::create_newsgroup(std::string name){
...
bool Inmemdb::delete_newsgroup(int id){
  int to_delete = -1;
...
news::Article Inmemdb::get_article(int ng_id, int article_id) {
...
bool Inmemdb::create_article(int ng_id, std::string title,std::string author, std::string text) {
  for (auto& ng : newsgroups) {
    if (ng.id == ng_id) {
...
bool Inmemdb::delete_article(int ng_id, int art_id) {
    for (int i = 0; i < newsgroups.size(); ++i) {
      news::Newsgroup& ng = newsgroups.at(i);
      if (ng.id == ng_id) {
...
bool Inmemdb::exists(std::string name){
...
std::vector<news::Article> Inmemdb::list_articles(int ng_id){
